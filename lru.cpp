...
int lru_replacementl3(cache* &C,int associativity)
{
	for(int i=0;i<associativity;i++)
	{
		if(!C[i].valid)
	...
	int index=0;
        for(int i=0;i<associativity;i++)
	{
		if(C[i].counter<C[index].counter)
		{
			index=i;
		}
	}
...
ull cache_handler_missl3(cache * &C,int associativity,int block_size,ull address, ull current_counter)
{
	...
	int replace_line=lru_replacementl3(C,associativity);
    ...
    for(int i=0;i<atoi(argv[2]);i++)
{
    ...
    while (!feof(fp))
    {
    	...
    	{
	    access_counter++;
    	    if(cache_presentl2(l2,l2_set,l2_associativity,l2_block_size,*addr,access_counter))
    	    {
    	        l2_hit_counter++;
    	    }
    	    else
    	    {
    	        l2_miss_counter++;
    	        if(cache_presentl3(l3,l3_associativity,l3_block_size,*addr,access_counter))
    	        {
    	            cache_handler_missl2(l2,l2_set,l2_associativity,l2_block_size,*addr,access_counter);
    	            l3_hit_counter++;
		}
    	        else
    	        {
    	            l3_miss_counter++;
    	            ull evicted_address = cache_handler_missl3(l3,l3_associativity,l3_block_size,*addr,access_counter);
